...
const std::string CLIENT_ID { "LoraToMQTT" };
const std::string TOPIC { "/wetterstation/aussen/luefterdrehzahl" };
...
int main(int argc, char* argv[])
{
	...
	std::string PersistDir("/home/pi/persist");
	mqtt::client client(SERVER_ADDRESS, CLIENT_ID, PersistDir);
	...
	try {
		...
		for(;;)
		{
			std::string pl = std::to_string(2000+(rand() % 2000));
			std::cout << "\nSending message..." << std::endl;
			auto pubmsg = mqtt::make_message(TOPIC, pl);
			pubmsg->set_qos(QOS);
			client.publish(pubmsg);
			std::cout << "...OK" << std::endl;
			std::this_thread::sleep_for(std::chrono::milliseconds(5000)); // sleep for x ms
		}
