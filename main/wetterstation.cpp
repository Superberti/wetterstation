...
// Pinbelegung:
// pin9=GPIO18=I2C0-SDA
// pin10=GPIO19=I2C0-SCL
...
extern "C"
{
  void app_main(void)
  {
    ...
    // TODO (pi#1#): Auf Knopfdruck (z.B. 10s) nvs_flash_erase() aufrufen und rebooten, damit wieder das WiFi konfiguriert werden kann.

    ESP_ERROR_CHECK(i2c_master_init(0));
    ESP_ERROR_CHECK(i2c_master_init(1));
    ESP_LOGI(TAG, "I2C OK");
...
#pragma GCC diagnostic pop
    ...
    // Bitfeld mit m�glichen Sensorfehlern
    ...
    // PCNT-Einheit z�hlt die Counterpulse vom CPU-L�fter
    ...
    for (;;)
    {
      ...
      if (veml_init_ok)
      {
        ...
        {
          RetrySuspectData = false;
          SensorStatus = veml.readLux(VEML_lux);
          if (c == 0)
            lux_mean = VEML_lux;
          if (SensorStatus != ESP_OK)
          {
...
void NRFLog(const char * const aLog, int len)
{
  gpio_set_level(GPIO_NUM_17, HIGH); // Write through einschalten
  // nicht warten, bis alles geschrieben werden konnte. Evtl. fehlt dann halt was
  uart_tx_chars(UART_NUM_1, aLog, len);
}
...
esp_err_t i2c_master_init(uint8_t aPortNum)
{
  ...
  return i2c_driver_install(aPortNum, conf.mode, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0);
}
...
void i2c_master_reset(uint8_t aPortNum)
{
  ...
  periph_module_disable(aPortNum == 0 ? PERIPH_I2C0_MODULE : PERIPH_I2C1_MODULE);
  periph_module_enable(aPortNum == 0 ? PERIPH_I2C0_MODULE : PERIPH_I2C1_MODULE);
  i2c_driver_delete(aPortNum);
  i2c_master_init(aPortNum);
}
